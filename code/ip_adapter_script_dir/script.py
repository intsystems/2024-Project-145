...
class ImageProcessor:
    ...
    def generate_images(self, output_dir, num_images, image_prompt_path, directory_name, gender):
        image = Image.open(image_prompt_path)
        image.resize((256, 256))
        images = self.ip_model.generate(pil_image=image, num_samples=num_images,
...
if __name__ == "__main__":
    ...
    sub_directories = [sub_dir.path for sub_dir in os.scandir(top_dirs_path) if sub_dir.is_dir()]
    ...
    for directory in sub_directories:
        ...
        if files:
            file_path = files[0]
            ip_processor.generate_images(args.save_path, args.num_images, files[0], directory_name, gender)
